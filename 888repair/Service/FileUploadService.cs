...
namespace _888repair.Service
{
    public class FileUploadService
    {
        ...
        public FlagTips FileLoad(HttpPostedFileBase httpPostedFileBase, string Stu_Empno)
        {
            ...
            if (httpPostedFileBase != null)
            {
                try
                {
                    ...
                    string result = SaveFile(fileData, fileName);//文件保存
        ...
        public string ITRepairPicDelete(string photoPath)
        {
            try
            {
                ...
                foreach (var fileName in photoPaths)
                {
                    //Directory.Delete(filePath, true); 删除目录   File.Delete(filePath); 删除文件
